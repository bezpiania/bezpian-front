...
// Configuración de campos por itemType (independiente del rubro)
const ITEM_TYPES = [
  { value: 'dish',    label: 'Plato',    icon: '🍽️', hint: 'Con ingredientes y alérgenos' },
  { value: 'product', label: 'Producto', icon: '📦', hint: 'Con stock y variantes' },
  { value: 'service', label: 'Servicio', icon: '🔬', hint: 'Con duración y especialidad' },
];

// Qué campos aplican según itemType
const ITEM_TYPE_FIELDS = {
  dish:    ['ingredients', 'allergens', 'tags', 'portionSize', 'prepTime', 'availableFor', 'isCombo'],
  product: ['sku', 'brand', 'barcode', 'stock', 'salePrice', 'weight', 'variants'],
  service: ['duration', 'specialty', 'requiresPrep', 'prepInstructions', 'insuranceCoverage', 'sessionCount'],
};
...
const ProductFormDynamic = ({ businessType, initial = {}, onSave, onCancel, saving }) => {
  ...
  // Detectar itemType inicial inteligentemente según el rubro si es un ítem nuevo
  const defaultItemType = initial.itemType || (businessType === 'restaurant' ? 'dish' : businessType === 'clinic' ? 'service' : 'product');
  ...
  const [form, setForm] = useState({
    ...
    isCombo: false, comboItems: [], comboSavings: '',
    ...initial,
  });

  const set = (key, val) => setForm(p => ({ ...p, [key]: val }));

  // Un campo se muestra si: el businessType lo permite Y el itemType lo incluye
  const showField = (key) => {
    if (fields[key]?.show === false) return false;
    const typeFields = ITEM_TYPE_FIELDS[form.itemType] || [];
    // Campos base siempre visibles
    const baseFields = ['name', 'description', 'price', 'category', 'subcategory', 'image', 'available'];
    if (baseFields.includes(key)) return true;
    return typeFields.includes(key);
  };

  const required = (key) => fields[key]?.required === true;
  const label = (key) => {
    // Adaptar labels según itemType
    if (key === 'name') {
      if (form.itemType === 'dish') return 'Nombre del plato';
      if (form.itemType === 'service') return 'Nombre del servicio';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Selector de tipo */}
      <div>
        <div className="field-label" style={{ marginBottom: 8 }}>Tipo de ítem</div>
          ...
          {ITEM_TYPES.map(t => (
            <button key={t.value} type="button"
              ...
              style={{
                ...
                border: `1.5px solid ${form.itemType === t.value ? 'var(--carbon)' : 'var(--rule)'}`,
                background: form.itemType === t.value ? 'var(--carbon)' : 'var(--bone)',
                color: form.itemType === t.value ? 'var(--bone)' : 'inherit',
                transition: 'all .15s',
              }}>
              <div style={{ fontSize: 18, marginBottom: 2 }}>{t.icon}</div>
              <div style={{ fontWeight: 600, fontSize: 13 }}>{t.label}</div>
              <div style={{ fontSize: 11, opacity: 0.6, marginTop: 1 }}>{t.hint}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      {showField('brand') && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          ...
          {showField('barcode') && <FieldWrap label={label('barcode')}><input style={inputStyle} value={form.barcode} onChange={e => set('barcode', e.target.value)} /></FieldWrap>}
          {showField('weight') && <FieldWrap label={label('weight')}><input style={inputStyle} type="number" step="0.01" value={form.weight} onChange={e => set('weight', e.target.value)} placeholder="0.5" /></FieldWrap>}
        </div>
      )}
      ...
      {(showField('portionSize') || show('prepTime') || show('availableFor')) && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
          {showField('portionSize') && (
